...
function ARROW_ENTITY.on_step(self, dtime)
	...
	-- Check for object "collision". Done every tick (hopefully this is not too stressing)
		...
		if closest_object then
			...
			minetest.log(tostring(lua))
			if obj == self._shooter and self._time_in_air > 1.02 or obj ~= self._shooter and (is_player or lua) then
	...
	if self._lastpos.x~=nil and not self._stuck then
		...
		if (math.abs(vel.x) < 0.0001) or (math.abs(vel.z) < 0.0001) or (math.abs(vel.y) < 0.00001) then
			...
			if not sdef or sdef.walkable == false or snode.name == "ignore" then
				...
				minetest.log(tostring(snode.name))
